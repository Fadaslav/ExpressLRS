...
function getPwmFormData() {
  ...
  while (inField = _(`pwm_${ch}_ch`)) {
    ...
    const wide = _(`pwm_${ch}_wide`).checked ? 1 : 0;
    ...
    const raw = (narrow << 19) | (wide << 22) | (mode << 15) | (invert << 14) | (inChannel << 10) | (failsafeMode << 20) | (failsafe - 988);
...
function updatePwmSettings(arPwm) {
  ...
  var pinModes = []
  // arPwm is an array of raw integers [49664,50688,51200]. 10 bits of failsafe position, 4 bits of input channel, 1 bit invert, 4 bits mode, 1 bit for narrow/750us, 1 bit for wide(500-2500us)
  const htmlFields = ['<div class="mui-panel pwmpnl"><table class="pwmtbl mui-table"><tr><th class="fixed-column">Output</th><th class="mui--text-center fixed-column">Features</th><th>Mode</th><th>Input</th><th class="mui--text-center fixed-column">Invert?</th><th class="mui--text-center fixed-column">750us?</th><th class="mui--text-center fixed-column">Wide?</th><th class="mui--text-center fixed-column pwmitm">Failsafe Mode</th><th class="mui--text-center fixed-column pwmitm">Failsafe Pos</th></tr>'];
  arPwm.forEach((item, index) => {
    ...
    htmlFields.push(`<tr><td class="mui--text-center mui--text-title">${index + 1}</td>
            ...
            <td><div class="mui-checkbox mui--text-center"><input type="checkbox" id="pwm_${index}_wide"${(wide) ? ' checked' : ''}></div></td>
  ...
  const setDisabled = (index, onoff) => {
    ...
    _(`pwm_${index}_wide`).disabled = onoff;
